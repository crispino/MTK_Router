...
int ont_edp_handle_get_transdata(ont_device_t *dev, const unsigned char* data,size_t len,ont_edp_device_t* device)
{
	SL_DEBUG();
	...
	SL_DEBUG();
    device->transdata_cb(dev, pkt.svr_name.value,pkt.svr_name.len,
...
int ont_edp_handle_get_pushdata(const unsigned char* data,size_t len,ont_edp_device_t* device)
{
    ...
    device->pushdata_cb(pkt.devid.value,pkt.devid.len,
	    pkt.data.value,pkt.data.len);
    return 0;
