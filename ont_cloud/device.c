...
#define ont_device_status_transform(dev, s) do {                        \
        SL_DEBUG("Device status transform: %d  ->  %d", dev->status, s); \
        dev->status = s;                                                \
    } while(0)
                               ...
                               const ont_device_rt_t *rt)
{
    ...
    SL_DEBUG("Connect to the server '%s:%u' successfully.",
                   dev->ip, dev->port);
                       ...
                       uint16_t keepalive)
{
    ...
    SL_DEBUG("Product id is %u, Device id is %u.",
                   dev->product_id,
                   dev->device_id);
    ...
    if (0 == dev->device_id)
    {
        ...
        SL_DEBUG("Begin to register the device ...");
        ...
        SL_DEBUG("Register the device successfully, device id is %u, key is '%s'.", dev->device_id, dev->key);
    }

    ont_device_status_transform(dev, ONTDEV_STATUS_RETRIEVING_ACCEPTOR);
#if 1
    err = ont_device_retrieve_server(dev, ip, port);
    SL_DEBUG("Retrieve the accceptor address successfully: %s:%u",
                   dev->ip, dev->port);
#endif
	SL_DEBUG("%d", err);
    if (ONT_ERR_OK == err)
                       ...
                       uint16_t keepalive)
{
    ...
    ONT_LOG_DEBUG3("Product id is %u, Device id is %u.auth_info is %s",
                   dev->product_id,
                   dev->device_id,
		   auth_info);
    SL_DEBUG("Product id is %u, Device id is %u.auth_info is %s",
                   dev->product_id,
                   dev->device_id,
		   auth_info);
                       ...
                       uint16_t keepalive)
{
    ...
    ont_platform_snprintf(dev->ip, sizeof(dev->ip), "%s", ip);
	SL_DEBUG("dev->ip: %s, ip: %s", dev->ip, ip);
    dev->port = port;
    dev->keepalive = keepalive;
