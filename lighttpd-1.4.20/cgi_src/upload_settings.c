...
#define WIFI3_CONF_STR "#WIFI3_CONF"
...
//	system(cmd);
	...
	while( len > 0){
		...
		if((data == '#') && (first_sharp == 0) && ((prev_data == '\r') || (prev_data == '\n'))){
			first_sharp = 1;
		...
		if((data == '#') && (first_sharp == 1)){
			...
			if (!strcmp(tmp_str, RT2860_CONF_STR)) {
				...
				fp = fp_rtdev;
			} else if(!strcmp(tmp_str, WIFI3_CONF_STR)){
				wifi3_conf_detect = 1;
				if(! fread(&data, 1, 1, src)) //read \n
					goto err;
				printf("Detect wifi3 config<br>\n");
				pname_wifi3 = tempnam("/var", "set");
				if(pname_wifi3 == NULL)
					pname_wifi3 = "/var/pname_wifi3";
				if(!(fp_wifi3 = fopen(pname_wifi3, "w+"))){
					printf("fopen(%s) NG<br>\n", pname_wifi3);
					goto err;
				}
				printf("Write wifi3 config to %s<br>\n", pname_wifi3);
				fp = fp_wifi3;
			} else {
				if(fp != NULL)
	...
	if (wifi3_conf_detect == 1){
		system("ralink_init clear wifi3");
		snprintf(cmd, 4096, "ralink_init renew wifi3 %s", pname_wifi3);
		system(cmd);
		free(pname_wifi3);
	}
	...
	if (wifi3_conf_detect == 1){
		printf("ralink_init clear wifi3<br>\n");
		printf("ralink_init renew wifi3 %s<br>\n", pname_wifi3);
		free(pname_wifi3);
	}
#endif
err:
