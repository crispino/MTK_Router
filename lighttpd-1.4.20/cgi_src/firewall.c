...
static void HeMu_DeleteIPfilterlist(char *input);
static void iptablesPROPortFilterClear(void);
#ifdef CONFIG_ALG
static void HeMu_ALG_iptalbes(void);
#endif
...
static void iptablesRemoteManagementRun(void)
{
	...
	if (atoi(spifw) == 0){
		...
		do_system("iptables -A %s -i %s -p tcp --dport 23 -j DROP", MALICIOUS_INPUT_FILTER_CHAIN , get_wanif_name());
	}
	else{
		do_system("iptables -A %s -i %s -m state -p tcp --dport 80 --state NEW,INVALID -j DROP", MALICIOUS_INPUT_FILTER_CHAIN, get_wanif_name());
		
		do_system("iptables -A %s -i %s -m state -p tcp --dport 23 --state NEW,INVALID -j DROP", MALICIOUS_INPUT_FILTER_CHAIN, get_wanif_name());
	}
	return;
}

static void makeIPMACWhiteListRule(char *buf, int len, char *mac_address,
...
char *dip_1, char *dip_2, int dprf_int, int dprt_int, int proto, int action)
{
	...
	case ACTION_DROP:
		...
		rc = snprintf(pos, len-rc, 
			"iptables -A %s ", IP_MAC_WHITELIST_CHAIN);
	    else
		rc = snprintf(pos, len-rc, 
			"iptables -A %s -m state --state NEW,INVALID ", IP_MAC_WHITELIST_CHAIN);
		break;
	case ACTION_ACCEPT:
		rc = snprintf(pos, len-rc, 
			"iptables -A %s ", IP_MAC_WHITELIST_CHAIN);
	...
	if(mac_address && strlen(mac_address)){
		// write interface
		/* for white list */
		rc = snprintf(pos, len-rc, "-i %s ", get_lanif_name());
		pos = pos+rc;
...
*/
		/* 白名单 */
		if (ACTION_ACCEPT == action)
		{
			makeIPMACWhiteListRule(cmd, sizeof(cmd), mac_address, sip_1, sip_2, sprf_int, sprt_int, dip_1, dip_2, dprf_int, dprt_int, proto, action);
			do_system(cmd);
		}
	...
	/* 需要白名单生效。所以将default policy加到最后的chain上 */
	switch(atoi(default_policy)){
	case 0:
		do_system("iptables -t filter -A %s -j ACCEPT", IP_MAC_WHITELIST_CHAIN);
		break;
	case 1:
		do_system("iptables -t filter -A %s -j DROP", IP_MAC_WHITELIST_CHAIN);
		break;
	}
...
static void iptablesIPPortFilterFlush(void)
{
	...
	do_system("iptables -F %s 1>/dev/null 2>&1", IP_MAC_WHITELIST_CHAIN);
}
static void HeMu_iptablesWebsFilterFlush(void)
{
	do_system("iptables -F %s 1>/dev/null 2>&1", WEB_FILTER_CHAIN);
	do_system("iptables -F %s 1>/dev/null 2>&1", URL_WHITELIST_CHAIN);
}
...
static void HeMu_MACFilter(char *input)
{
	char rule[8192];
	char *mac_address;
	char *sip_1, *sip_2, *sprf, *sprt, *protocol, *action_str, *comment;
	...
	char *enable;
	enable = strdup(web_get("apselect_0", input, 1));
	DBG_MSG("enable: %s", enable);
	if (enable != NULL)
		nvram_set(RT2860_NVRAM, "MACFilterEn", enable);
	free_all(1, enable);
...
static void HeMu_IPFilter(char *input)
{
	char rule[8192];
	char *mac_address;
	char *sip_1, *sip_2, *sprf, *sprt, *protocol, *action_str, *comment;
	...
	DBG_MSG("enable: %s", enable);
	if (enable != NULL)
		nvram_set(RT2860_NVRAM, "IPFilterEn", enable);

	char *filter_type;
	filter_type = strdup(web_get("filter_type", input, 1));
	DBG_MSG("filter_type: %s", filter_type);
	if (filter_type != NULL)
		nvram_set(RT2860_NVRAM, "filter_type", filter_type);

	free_all(2, enable, filter_type);

	iptablesPROPortFilterClear();
...
static void iptablesPROPortFilterRun(void)
{
	char cmd_buf[256] = {0};
	char *IPFilterEn = (char *)nvram_bufget(RT2860_NVRAM, "IPFilterEn");
	char *filter_type = (char *)nvram_bufget(RT2860_NVRAM, "filter_type");
	char *filter_protocol = (char *)nvram_bufget(RT2860_NVRAM, "filter_protocol");

	if (NULL == IPFilterEn || NULL == filter_type || NULL == filter_protocol)
		return ;

	/* IP过滤没有关闭，且过滤类型为基于协议过滤。 */
	if (0 != strcmp("0", IPFilterEn) && 0 == strcmp("1", filter_type))
	{
		/* ICMP+ARP */
		if (!strcmp(filter_protocol, "0"))
		{
			snprintf(cmd_buf, sizeof(cmd_buf), "iptables -I %s -d any/0 -s any/0 -p icmp -j DROP", IPPORT_FILTER_CHAIN);
			do_system(cmd_buf);
			memset(cmd_buf, 0x0, sizeof(cmd_buf));
			snprintf(cmd_buf, sizeof(cmd_buf), "arptables -A INPUT -i %s -p arp -j DROP", get_wanif_name());
			do_system(cmd_buf);
		}
		/* ICMP */
		else if (!strcmp(filter_protocol, "1"))
		{
			snprintf(cmd_buf, sizeof(cmd_buf), "iptables -I %s -d any/0 -s any/0 -p icmp -j DROP", IPPORT_FILTER_CHAIN);
			do_system(cmd_buf);
		}
		/* ARP */
		else if (!strcmp(filter_protocol, "2"))
		{
			snprintf(cmd_buf, sizeof(cmd_buf), "arptables -A INPUT -i %s -p arp -j DROP", get_wanif_name());
			do_system(cmd_buf);
		}
...
static void HeMu_PROFilter(char *input)
{
	char cmd_buf[256] = {0};

	char *enable, *filter_type, *filter_protocol;
	enable = filter_type = filter_protocol = NULL;

	enable = strdup(web_get("apselect_0", input, 1));
	filter_type = strdup(web_get("filter_type", input, 1));
	filter_protocol = strdup(web_get("filter_protocol", input, 1));

	DBG_MSG("enable %s", enable);
	DBG_MSG("filter_type %s", filter_type);
	DBG_MSG("protocol %s", filter_protocol);

	if (enable != NULL)
		nvram_bufset(RT2860_NVRAM, "IPFilterEn", enable);

	if (filter_type != NULL)
		nvram_bufset(RT2860_NVRAM, "filter_type", filter_type);

	if (filter_protocol != NULL)
		nvram_bufset(RT2860_NVRAM, "filter_protocol", filter_protocol);

	nvram_commit(RT2860_NVRAM);

	/* 开启协议过滤，则删除所有IP过滤规则 */
	HeMu_DeleteIPfilterlist(input);

	iptablesPROPortFilterClear();
	iptablesPROPortFilterRun();

	free_all(3, enable, filter_type, filter_protocol);
...
static void HeMu_iptablesWebsFilterRun()
{
	int i;
	char entry[256];
	char url[64] = {0};
	char rec[4] = {0};
	char *url_filter = (char *)nvram_bufget(RT2860_NVRAM, "HeMu_URLFilters");
	...
	while( (get_nth_value(i++, url_filter, ';', entry, sizeof(entry)) != -1) ){
		if(strlen(entry)){
			...
			// get url
			if((get_nth_value(0, entry, ',', url, sizeof(url)) == -1))
				continue;

			// get action
			if((get_nth_value(1, entry, ',', rec, sizeof(rec)) == -1))
				continue;

			if (ACTION_DROP == atoi(rec))
				do_system("iptables -A " WEB_FILTER_CHAIN " -p tcp -m tcp -m webstr --url %s -j REJECT --reject-with tcp-reset", url);
			else if (ACTION_ACCEPT == atoi(rec))
			{
				do_system("iptables -A " WEB_FILTER_CHAIN " -p tcp -m tcp -m webstr --url %s -j ACCEPT", url);
				do_system("iptables -A " URL_WHITELIST_CHAIN " -i %s -p tcp -m tcp -m webstr ! --url %s -j REJECT --reject-with tcp-reset", get_lanif_name(), url);
			}
		}
	}
...
static void HeMu_URLFilter(char *input)
{
	char *urlfilters = (char *)nvram_bufget(RT2860_NVRAM, "HeMu_URLFilters");
	//char *rule = websGetVar(wp, T("addURLFilter"), T(""));
	char *rule;
	char rule_buf[64] = {0};
	char *action_str;
	...
	DBG_MSG("enable: %s", enable);
	if (enable != NULL)
		nvram_set(RT2860_NVRAM, "URLFilterEn", enable);
	free_all(1, enable);

	rule = strdup(web_get("addURLFilter", input, 1));
	...
	snprintf(rule_buf, sizeof(rule_buf), "%s,%d", rule, action);

	if(!urlfilters || !strlen(urlfilters)){

		nvram_bufset(RT2860_NVRAM, "HeMu_URLFilters", rule_buf);
	}
	else{
		if(! (new_urlfilters = (char *)malloc(sizeof(char) * (strlen(urlfilters)+strlen(rule_buf)+2))))
			return;

		new_urlfilters[0] = '\0';
		strcat(new_urlfilters, urlfilters);
		strcat(new_urlfilters, ";");
		strcat(new_urlfilters, rule_buf);
		nvram_bufset(RT2860_NVRAM, "HeMu_URLFilters", new_urlfilters);
		free(new_urlfilters);
	}
	nvram_commit(RT2860_NVRAM);
	...
	/* TODO: check YY */
	/* Flush iptables */
	HeMu_iptablesWebsFilterFlush();

	HeMu_iptablesWebsFilterRun();
	//printf("add URL filter: %s<br>\n",rule);
...
static void iptablesHeMuWebWhiteListClear(void)
{
	do_system("iptables -D FORWARD -j %s  1>/dev/null 2>&1", URL_WHITELIST_CHAIN);
	do_system("iptables -F %s  1>/dev/null 2>&1", URL_WHITELIST_CHAIN);
}
static void iptablesHeMuIPMACWhiteListClear(void)
{
	do_system("iptables -D FORWARD -j %s  1>/dev/null 2>&1", IP_MAC_WHITELIST_CHAIN);
	do_system("iptables -F %s  1>/dev/null 2>&1", IP_MAC_WHITELIST_CHAIN);
}
static void iptablesPROPortFilterClear(void)
{
	do_system("arptables -F 1>/dev/null 2>&1");
}
...
static void iptablesAllFilterClear(void)
{
	...
	iptablesHeMuWebWhiteListClear();
	iptablesHeMuIPMACWhiteListClear();
	iptablesPROPortFilterClear();
...
//add by luozf add ALG funciton
static void HeMu_ALG(char *input)
{
	WebTF(input, "ftpEn", RT2860_NVRAM, "ALG_ftp", 1);
	WebTF(input, "sipEn", RT2860_NVRAM, "ALG_sip", 1);
	WebTF(input, "rtspEn", RT2860_NVRAM, "ALG_rtsp", 1);
	WebTF(input, "l2tpEn", RT2860_NVRAM, "ALG_l2tp", 1);
	
	nvram_commit(RT2860_NVRAM);
	do_system("init_system restart");
}

static void HeMu_ALG_iptalbes()
{
	char *ALG_rtsp;
	int int_ALG_rtsp;
	...
	ALG_rtsp = (char *)nvram_bufget(RT2860_NVRAM, "ALG_rtsp");
	if(!ALG_rtsp){
	  DBG_MSG("Warning: can't find \"ALG_rtsp\" in flash.");
	  return;
	}
	int_ALG_rtsp = atoi(ALG_rtsp);
	//fuck 内核模块限制不了，只好用layer7要限制
	if( 0 == int_ALG_rtsp)
	{
		//禁用后添加
		//do_system("iptables -I FORWARD -m layer7 --l7dir /etc_ro/l7-protocols --l7proto rtsp -j DROP");
		do_system("iptables -I FORWARD -p tcp --dport 8554 -j DROP");
		do_system("iptables -I FORWARD -p tcp --dport 544 -j DROP");
	}
	else
	{
		//do_system("iptables -D FORWARD -m layer7 --l7dir /etc_ro/l7-protocols --l7proto rtsp -j DROP");
		do_system("iptables -D FORWARD -p tcp --dport 8554 -j DROP");
		do_system("iptables -D FORWARD -p tcp --dport 544 -j DROP");
	}


	ALG_sip = (char *)nvram_bufget(RT2860_NVRAM, "ALG_sip");
	if(!ALG_sip){
	  DBG_MSG("Warning: can't find \"ALG_sip\" in flash.");
	  return;
	}
	int_ALG_sip = atoi(ALG_sip);
	//fuck 内核模块限制不了，只好用iptables
	if( 0 == int_ALG_sip )
	{
		//禁用后添加
		do_system("iptables -I FORWARD -p udp --dport 5060 -j DROP");		
	}
	else
	{
		do_system("iptables -D FORWARD -p udp --dport 5060 -j DROP");
	}

	
	ALG_l2tp = (char *)nvram_bufget(RT2860_NVRAM, "ALG_l2tp");
	if(!ALG_l2tp){
		  DBG_MSG("Warning: can't find \"ALG_l2tp\" in flash.");
		  return;
	}
	int_ALG_l2tp = atoi(ALG_l2tp);
	//fuck 内核模块限制不了，只好用iptables
	if( 0 == int_ALG_l2tp)
	{
		//禁用后添加
		do_system("iptables -I FORWARD -p udp --dport 1701 -j DROP");
		do_system("iptables -I FORWARD -p udp --dport 500 -j DROP");	
		do_system("iptables -I FORWARD -p udp --dport 4500 -j DROP");	
	}
	else
	{
		do_system("iptables -D FORWARD -p udp --dport 1701 -j DROP");
		do_system("iptables -D FORWARD -p udp --dport 500 -j DROP");	
		do_system("iptables -D FORWARD -p udp --dport 4500 -j DROP");		
	}
...
void firewall_init(void)
{
	...
	do_system("iptables -t filter -N %s 1>/dev/null 2>&1", URL_WHITELIST_CHAIN);
	do_system("iptables -t filter -N %s 1>/dev/null 2>&1", IP_MAC_WHITELIST_CHAIN);
	...
	do_system("iptables -t filter -A FORWARD -j %s 1>/dev/null 2>&1", URL_WHITELIST_CHAIN);
	do_system("iptables -t filter -A FORWARD -j %s 1>/dev/null 2>&1", IP_MAC_WHITELIST_CHAIN);
...
static void HeMu_DeleteURLfilterlist(char *input)
{
	int i, j, rule_count;
	char del_buf[64];
	char rec[4];
    ...
    const char *rules = (char *)nvram_bufget(RT2860_NVRAM, "HeMu_URLFilters");
    if(!rules || !strlen(rules) )
        return;
	...
	if (get_nth_value(1, input, '&', del_buf, sizeof(del_buf)) == -1)
		DBG_MSG("get del buf failed");

	i = 0;
	j = 0;
	DBG_MSG("input: %s", del_buf);
	while((get_nth_value(i++, del_buf, ';', rec, sizeof(rec)) != -1))
	{
	...
	nvram_set(RT2860_NVRAM, "HeMu_URLFilters", new_rules);
	nvram_commit(RT2860_NVRAM);
...
static void HeMu_DeleteMACfilterlist(char *input)
{
	int i, j, rule_count;
	char rec[4];
	char del_buf[64];
...
#ifdef CONFIG_ALG
//add by luozf add ALG funciton
		else if (!strcmp(firewall, "HeMu_ALG")){
		HeMu_ALG(inStr);
	}
#endif
	//end of add
	else if (!strcmp(firewall, "HeMu_URLfilter")){
		HeMu_URLFilter(inStr);
	} else if (!strcmp(firewall, "HeMu_URLFilterDelete")){
		HeMu_DeleteURLfilterlist(inStr);
	} else if (!strcmp(firewall, "HeMu_MACfilter")){
		HeMu_MACFilter(inStr);
	} else if (!strcmp(firewall, "HeMu_MACFilterDelete")){
		HeMu_DeleteMACfilterlist(inStr);
	} else if (!strcmp(firewall, "HeMu_IPfilter")){
		HeMu_IPFilter(inStr);
	} else if (!strcmp(firewall, "HeMu_IPFilterDelete")){
		HeMu_DeleteIPfilterlist(inStr);
	} else if (!strcmp(firewall, "HeMu_PROFilter")){
		HeMu_PROFilter(inStr);
	}
