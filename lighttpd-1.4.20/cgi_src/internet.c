...
void set_hemu_lan(char *input)
{
	...
	nvram_bufset(RT2860_NVRAM, "dhcpGateway", ip);
...
static void adv_wan_init()
{
	const char *upnpEn;

	do_system("killall -9 miniupnpd");

	upnpEn = nvram_bufget(RT2860_NVRAM, "upnpEnabled");
	if (!upnpEn || !strlen(upnpEn))
		return ;
...
void set_hemu_wan(char *input)
{
    ...
    //TODO: MTU
    WebTF(input, "MTU", RT2860_NVRAM, "wan_pppoe_mtu", 1);

	nvram_commit(RT2860_NVRAM);
	do_system("init_system restart");
}

void set_hemu_wan_adv(char *input)
{
	web_debug_header();
	WebTF(input, "igmpSnoopingEn", RT2860_NVRAM, "M2UEnabled", 1);
	WebTF(input, "igmpproxyEn", RT2860_NVRAM, "igmpEnabled", 1);
	WebTF(input, "UpnpEn", RT2860_NVRAM, "upnpEnabled", 1);
	
	WebTF(input, "vlanEn", RT2860_NVRAM, "VLANEnable", 1);
	WebTF(input, "vlanid", RT2860_NVRAM, "VLANID", 1);
	WebTF(input, "vlanpriority", RT2860_NVRAM, "VLANPriority", 1);
...
//add by luozf 
void set_hemu_dhcp(char *input)
{
	char *dhcp_tp, *dhcp_s, *dhcp_e;

	dhcp_tp = dhcp_s = dhcp_e  = NULL;
	
	dhcp_tp = strdup(web_get("dhcphidden", input, 1));
	dhcp_s = strdup(web_get("starthidden", input, 1));
	dhcp_e = strdup(web_get("endhidden", input, 1));


	if( 0 == atoi(dhcp_tp))
		nvram_bufset(RT2860_NVRAM, "dhcpEnabled", "0");
	else if ( 1 == atoi(dhcp_tp)) {
		if (-1 == inet_addr(dhcp_s)) {
			DBG_MSG("invalid DHCP Start IP");
			goto leave;
		}
	...
	//do_system("init_system restart");
leave: 
	free_all(3, dhcp_tp, dhcp_s, dhcp_e);
	return;
}
void set_hemu_mac_clone(char *input)
{
...
//added by chenzhuo for Andmu WEB WAN Setting
	} else if (!strcmp(form, "HeMu_Wan")) {
		set_hemu_wan(inStr);
	} else if (!strcmp(form, "HeMu_Wan_Adv")) {
		set_hemu_wan_adv(inStr);
//ended by chenzhuo for Andmu WEB WAN Setting
#if defined CONFIG_IPV6
	} else if (!strcmp(form, "ipv6")) {
		set_ipv6(inStr);
#endif
//add by  luozf 添加适配和目路由界面2017.11.15
	}else if (!strcmp(form, "HeMu_dhcp")) {
		set_hemu_dhcp(inStr);
		set_hemu_lan(inStr);
		free(inStr);
	...
	else if (!strcmp(form, "HeMu_MacClone")) {
		set_hemu_mac_clone(inStr);
		free(inStr);
		nvram_close(RT2860_NVRAM);
		return 0;
	}
