...
<body onload=\"update()\"> <h1> Upload Firmware</h1>");
	...
	memset(buf, 0, ONE_SEGMENT);
	total_len = 0;
	while(read_len = fread(buf, 1, ONE_SEGMENT, stdin))
	{
		file_end = (char*)memmem(buf, read_len, boundary, boundary_len);
		/* Not file end */
		if (NULL == file_end)
		{
		...
		else //The end
		{
			...
			fwrite(buf, 1, file_end - buf, fp);
			total_len += file_end - buf;
		}
	...
	// examination
#if defined (UPLOAD_FIRMWARE_SUPPORT)
		if(!check(UPLOAD_FILE, 0, total_len, err_msg) ){
