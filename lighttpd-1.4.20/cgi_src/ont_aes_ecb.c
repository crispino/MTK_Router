...
void ont_aes128_ecb_pkcs5_encrypt(unsigned char *key, unsigned char *input, unsigned char *output)
{
	...
	MD5_Init(&md5c);
	MD5_Update(&md5c, key, strlen(key));
...
void ont_aes128_ecb_pkcs5_decrypt(unsigned char *key, unsigned char *input, unsigned char *output)
{
	unsigned char digest[16] = {0};
	MD5_CTX md5c;
    unsigned char aes_data[512] = {0};
