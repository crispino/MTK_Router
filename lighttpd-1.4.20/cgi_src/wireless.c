...
void andmu_wizard_set_wifi(int nvram, char *input)
{
	//配置过，不再进入快速设置页面。
	nvram_bufset(RT2860_NVRAM, "noNeedWizard", "1");

	andmu_wizard_set_wifi_innner(RT2860_NVRAM, input);
#if defined CONFIG_RTDEV
	nvram_init(RTDEV_NVRAM);
	andmu_wizard_set_wifi_innner(RTDEV_NVRAM, input);
#endif

	do_system("init_system restart");
}
	
void andmu_wizard_set_wifi_innner(int nvram, char *input)
{
	char *wifi_switch, *security_mode, *passphrase, *mssid_0,*mssid_1, *wifi_strength, *hssid, *hssid_5G;
	char sTxPower[4] = {0};
	char sHssid[2] = {0};
	char sHssid_5G[2] = {0};
	char sWifiOff[32] = {0};
	...
	wifi_switch = web_get("wifihiddenButton", input, 1);
//	DBG_MSG("wifi_switch[%s], input params[%s]",  wifi_switch, input);	
	if (!strcmp(wifi_switch, "0")) { //Close Wifi
		sprintf(system, "ifconfig %s down", eth);
		do_system(system);
		nvram_bufset(nvram, "WiFiOff", "1");
		nvram_commit(nvram);
		do_system("init_system restart");
	...
	hssid = web_get("hssid", input, 1);
	if (strlen(hssid) > 0) {
		strcpy(sHssid, "1");
	} else {
		strcpy(sHssid, "0");
	}
	hssid_5G = web_get("hssid_5G", input, 1);
	if (strlen(hssid_5G) > 0) {
		strcpy(sHssid_5G, "1");
	} else {
		strcpy(sHssid_5G, "0");
	}
	...
	wifi_strength = web_get("wifi_strength", input, 1);
	...
	DBG_MSG("sTxPower[%s]",  sTxPower);
	
	security_mode = strdup(web_get("security_mode", input, 1));
	//DBG_MSG("security_mode[%s]", security_mode);
	passphrase = strdup(web_get("passphrase", input, 1));
	//DBG_MSG("passphrase[%s]", passphrase);
	
	mssid_0 = strdup(web_get("mssid_0", input, 1));
	mssid_1 = strdup(web_get("mssid_0_5G", input, 1));


	// 1. ssid
	if (nvram == RT2860_NVRAM) {
		nvram_bufset(nvram, "SSID1", mssid_0);
	} else {
		nvram_bufset(nvram, "SSID1", mssid_1);
	}

	//2. hide ssid
	if (nvram == RT2860_NVRAM) {
		set_nth_value_flash(nvram, 0, "HideSSID", sHssid);
	} else {
		set_nth_value_flash(nvram, 0, "HideSSID", sHssid_5G);
	}	
	//3. transitPower 
	nvram_bufset(nvram, "TxPower", sTxPower);

	//5. auth mode configuration
	if (!strcmp(security_mode, "WEP")) {
		
		//1. default Key
		char *sKeyID = web_get("wep_default_key", input, 1);
		int nKeyID = atoi(sKeyID);
		set_nth_value_flash(nvram, 0, "DefaultKeyID", sKeyID);
		//2. Key Str
		char szKeyStr[8] = {0};
		sprintf(szKeyStr, "Key%dStr1", nKeyID);
		WebTF(input, "wep_key1", nvram, szKeyStr, 1);
		//3. Key Type
		char szKeyType[8] = {0};
		sprintf(szKeyType, "Key%dType", nKeyID);
		DBG_MSG("keyID[%d], KeyStr[%s], KeyType[%s]!", 
									nKeyID, szKeyStr, szKeyType);
		WebNthTF(input, "wep_Select", nvram, 0, szKeyType, 1);
		
		set_nth_value_flash(nvram, 0, "AuthMode", "OPEN");
		set_nth_value_flash(nvram, 0, "EncrypType", "WEP");
		...
		set_nth_value_flash(nvram, 0, "AuthMode", security_mode);
		nvram_bufset(nvram,  "WPAPSK1", passphrase);

		char *cipher_str = web_get("cipher", input, 1);
		set_nth_value_flash(nvram, 0, "EncrypType", cipher_str);

		// Set Other Configuration Default Value
		set_nth_value_flash(nvram, 0, "RekeyMethod", "TIME");
		set_nth_value_flash(nvram, 0, "IEEE8021X", "0");
		set_nth_value_flash(nvram, 0, "DefaultKeyID", "2");
		set_nth_value_flash(nvram, 0, "RekeyInterval", "3600");
		
	} else { // Open-None Mode
		set_nth_value_flash(nvram, 0, "AuthMode", "OPEN");
		set_nth_value_flash(nvram, 0, "EncrypType", "NONE");
	}
	
	//6. 设置wfii总开关
	memset(system, 0, sizeof(system));
	sprintf(system, "ifconfig %s up", eth);
	do_system(system);
	nvram_bufset(nvram, "WiFiOff", "0");

	//7. admin password configure

	nvram_commit(nvram);

	//free memory
	free_all(3, security_mode, passphrase, mssid_0);
	return;	
...
void andmu_set_wifi_security(int nvram, char *input)
{
	char *wifi_switch, *security_mode, *passphrase, *mssid_0, *wifi_strength, *hssid, *gbrand;
	char sTxPower[4] = {0};
	char sHssid[2] = {0};
	char sWifiOff[32] = {0};
	char system[32] = {0};
	char eth[8] = {0};
	int flag2or5=0;
	gbrand = web_get("wlan_conf", input, 1);
	if (!strcmp(gbrand, "2860")) {
		flag2or5 =0;
		strcpy(eth, "ra0");
	} else {
		flag2or5 =1;
	...
	passphrase = strdup(web_get("passphrase", input, 1));
	mssid_0 = strdup(web_get("mssid_0", input, 1));
	DBG_MSG("security_mode[%s], hssid[%s], passphrase[%s], mssid_0[%s], wifi_strength[%s]!", 
				security_mode, sHssid, passphrase, mssid_0, sTxPower);
	// 1. ssid
	nvram_bufset(nvram, "SSID1", mssid_0);
	...
	//4. channel and bandwidth configure
	do {
		char *sz11gChannel = strdup(web_get("channel", input, 1));
		if(0 == strlen(sz11gChannel)) {
			free(sz11gChannel);
			break;
		}

		char *szBnadWidth = strdup(web_get("n_bandwidth", input, 1));
		if(0 == strlen(szBnadWidth)) {
			free(szBnadWidth);
			break;
		}

		char *szCoExit = strdup(web_get("n_2040_coexit", input, 1));
		if(0 == strlen(szCoExit)) {
			free(szCoExit);
			break;
		}

		DBG_MSG("channel[%s], bandwidth[%s], coexit[%s]!", 
							sz11gChannel, szBnadWidth, szCoExit);
		...
		do_system("iwpriv %s set Channel=%s", eth, sz11gChannel);

		nvram_bufset(nvram, "HT_BW", szBnadWidth);

		if(!strncmp(szBnadWidth, "0", 2) && flag2or5 == 1 )		// set bgn mode
			{
			nvram_bufset(nvram, "HT_BW", szCoExit);
			nvram_bufset(nvram, "HT_EXTCHA", "0");
			nvram_bufset(nvram, "VHT_BW", "0");
			nvram_bufset(nvram, "HT_BSSCoexistence", "0");
//			nvram_bufset(nvram, "VHT_STBC", "0");
//			nvram_bufset(nvram, "VHT_SGI", "0");
//			nvram_bufset(nvram, "VHT_BW_SIGNAL", "0");
			}
		else if(flag2or5 == 1 )
			{
			nvram_bufset(nvram, "HT_EXTCHA", "1");
			nvram_bufset(nvram, "VHT_BW", "1");
//			nvram_bufset(nvram, "VHT_STBC", "1");
//			nvram_bufset(nvram, "VHT_SGI", "1");
//			nvram_bufset(nvram, "VHT_BW_SIGNAL", "1");
			}
		
		if(atoi(szCoExit) == 1)
			nvram_bufset(nvram, "ht_2040_flag", "1");
		else
			nvram_bufset(nvram, "ht_2040_flag", "0");

		if(flag2or5 == 0)
			nvram_bufset(nvram, "HT_BSSCoexistence", "0");  //force 40M
			
		free_all(3, sz11gChannel, szBnadWidth, szCoExit);
...
void set_wifi_security_basic(int nvram, char *input)
{
	char 	*value;
	int 	mbssid;
	char 	*security_mode;
...
#if defined (CONFIG_PRODUCT_AN1303) 
	//mapping the 40Mhz to HT_BW:1; HT_BSSCoexistence:0
	//setting HT_BSSCoexistence to 0 is done by JavaScript in basic.shtml
	value = web_get("n_bandwidth", input, 0);
	if (!strncmp(value, "2", 2)) {
		nvram_bufset(nvram, "HT_BW", "1");
	}

	//mapping broadcastssid to HideSSID
	value = web_get("broadcastssid", input, 0);
	if (!strncmp(value, "0", 2)) {
		set_nth_value_flash(nvram, 0, "HideSSID", "1");
	}
	
	set_wifi_security_basic(RT2860_NVRAM, input);
#endif
	do_system("init_system restart");
...
{
	char  	*ssid=NULL,*channel=NULL,*authmode=NULL; 
	DBG_MSG("**nvram:%d, *****%s**********", nvram, input);
	...
	if (nvram == RTDEV_NVRAM) {
		...
		do_system("iwpriv apcli0 set ApCliSsid=%s", "");
	} else {
		nvram_init(RTDEV_NVRAM);
		nvram_bufset(RTDEV_NVRAM, "ApCliSsid", "");
		nvram_bufset(RTDEV_NVRAM, "ApCliEnable", "0");
		nvram_commit(RTDEV_NVRAM);
		do_system("iwpriv apclii0 set ApCliEnable=%d", 0);
		do_system("iwpriv apclii0 set ApCliBssid=");
		do_system("iwpriv apclii0 set ApCliSsid=%s", "");
	}
#endif

	nvram_bufset(nvram, "ApCliSsid", ssid);
	
	channel = web_get("ra0channel", input, 1);
	nvram_bufset(nvram, "Channel", channel);
	nvram_bufset(nvram, "AutoChannelSelect", "0");
	//nvram_bufset(RT2860_NVRAM, "ApCliEnable", "1");
	WebTF(input, "apcliEnbl", nvram, "ApCliEnable", 1);
	WebTF(input, "apcli_bssid", nvram, "ApCliBssid", 1);
	WebTF(input, "apcli_mode", nvram, "ApCliAuthMode", 1);

	authmode = web_get("apcli_mode", input, 1);
	if(strstr(authmode,"WEP"))
		nvram_bufset(nvram, "ApCliEncrypType","WEP");
	else if(strstr(authmode,"OPEN"))
		nvram_bufset(nvram, "ApCliEncrypType","NONE");
	else
	...
	WebTF(input, "apcli_key3type", nvram, "ApCliKey3Type", 1);
	WebTF(input, "apcli_key4type", nvram, "ApCliKey4Type", 1);
	WebTF(input, "apcli_wpapsk", nvram, "ApCliKey1Str", 1);
	WebTF(input, "apcli_key2", nvram, "ApCliKey2Str", 1);
	WebTF(input, "apcli_key3", nvram, "ApCliKey3Str", 1);
	WebTF(input, "apcli_key4", nvram, "ApCliKey4Str", 1);
#endif
...
static void wps_ap_pbc_start_all(int nvram)
{
	...
	if (nvram == RTDEV_NVRAM) {
		if(access("/tmp/rai0",0) != 0)
			do_system("echo wpsbuttonpress > /tmp/rai0");
		strcpy(iface, "rai0");	
	}
#endif
...
#endif
//add by luozf 适配和目路由界面 2017.11.14
void set_HeMu_channel(int nvram, char *input)
{
	char *sz11bChannel, *sz11gChannel;
	char ifprefix[4];
	...
	sz11bChannel = strdup(web_get("sz11bChannel", input, 0));
	sz11gChannel = strdup(web_get("sz11gChannel", input, 0));

	//11abg Channel or AutoSelect
	if ((0 == strlen(sz11bChannel)) && (0 == strlen(sz11gChannel))) {
		DBG_MSG("'Channel' should not be empty!");
		goto leave;
	}
	if ( !strncmp(sz11bChannel, "0", 2) || !strncmp(sz11gChannel, "0", 2))
		nvram_bufset(nvram, "AutoChannelSelect", "1");
	else
		nvram_bufset(nvram, "AutoChannelSelect", "0");
	...
	do_system("init_system restart");
leave:
	free_all(2,sz11bChannel, sz11gChannel);

}

void set_HeMu_controlMacList(int nvram, char *input)
{
	char *sz11bChannel, *sz11gChannel;
	char ifprefix[4];
	char *apselect, *newap_list;
	...
	apselect = web_get(racat("apselect_", mbssid), input, 0);
	if(!strlen(apselect)){
		DBG_MSG("cant find %s", apselect);
		goto leave;
	}
	...
	newap_list = web_get(racat("newap_text_", mbssid), input, 0);
	if(!newap_list)
#if defined (CONFIG_PRODUCT_AN1303) 
		goto submit_leave;
#else
...
void set_HeMu_wifiGuest(int nvram, char *input)
{
	char *value;
	...
	if (!strncmp(value, "0", 2)) {
		...
		int bssid_num = strtol(nvram_bufget(nvram, "BssidNum"), NULL, 10);
		if(bssid_num >= 2)
			do_system("ifconfig %s%d down", ifprefix, 1);
#endif
		nvram_set(nvram, "Guest_off", "1");
		nvram_commit(nvram);
		do_system("/sbin/wifi_guest.sh");
		return;//访客网络关闭则不需要设置其他的参数
	}
	else if (!strncmp(value, "1", 2)) {
#if defined (RT2860_MBSS_SUPPORT) || defined (RTDEV_MBSS_SUPPORT)
		int bssid_num = strtol(nvram_bufget(nvram, "BssidNum"), NULL, 10);
		if(bssid_num >= 2)
			do_system("ifconfig %s%d up", ifprefix, 1);
#endif
		nvram_set(nvram, "Guest_off", "0");
	}
	...
	hssid = ssid = NULL;
	hssid = strdup(web_get("hssid_1", input, 1));
	ssid = strdup(web_get("mssid_1", input, 1));
	
	nvram_bufset(nvram, racat("SSID", 2), ssid);

	set_nth_value_flash(nvram, 1, "HideSSID", hssid);

	nvram_commit(nvram);
	do_system("init_system restart");
leave:
	free_all(2,hssid,  ssid);
...
#endif
	} else if(!strcmp(page, "wireless_setting")) {
		andmu_set_wifi_security(nvram_id, inStr);
	}
	else if(!strcmp(page, "wireless_QuickSetwifi")) {
		andmu_wizard_set_wifi(nvram_id, inStr);
	}
//add by luozf 适配和目路由界面 2017.11.14
	else if (!strcmp(page, "HeMu_channel")) {
		 set_HeMu_channel(nvram_id, inStr);
	}
	else if (!strcmp(page, "HeMu_maclist")) {
		 set_HeMu_controlMacList(nvram_id, inStr);
	}
	else if (!strcmp(page, "HeMu_guest")) {
		 set_HeMu_wifiGuest(nvram_id, inStr);
	}
//end of add
	free(inStr);
