...
{
#if defined (CONFIG_RALINKAPP_SWQOS) || defined (CONFIG_CUSTOM_QOS) 
...
//add by luozf 2017.11.7
inline void qos_restart_zw(void)
{
	do_system("init_system restart");
}

static void qos_zowee(char *input)
{
	char *qos_enable, *upload_bandwidth, *download_bandwidth, 
		 *upload_bandwidth_custom, *download_bandwidth_custom;
	
	qos_enable  = upload_bandwidth = download_bandwidth 
		= upload_bandwidth_custom = download_bandwidth_custom = NULL;
	web_debug_header();
	...
	if(!strcmp(qos_enable, "1")/* bi-dir */){
		if(!strlen(upload_bandwidth))
			goto leave;
		if(!strlen(download_bandwidth))
			goto leave;
	}
	nvram_bufset(RT2860_NVRAM, "QoSEnable_zw", qos_enable);
	...
	if(!strcmp(qos_enable, "1") /* bi-dir */){
		...
		if(!strchr(postfix, 'k') && !strchr(postfix, 'K')  && !strchr(postfix, 'm') && !strchr(postfix, 'M') )
			strncat(postfix, "k", sizeof(postfix));
		nvram_bufset(RT2860_NVRAM, "QoSUploadBandwidth_custom_zw", postfix);
		strncpy(postfix, download_bandwidth_custom, sizeof(postfix));
		if(!strchr(postfix, 'k') && !strchr(postfix, 'K')  && !strchr(postfix, 'm') && !strchr(postfix, 'M') )
			strncat(postfix, "k", sizeof(postfix));

		nvram_bufset(RT2860_NVRAM, "QoSDownloadBandwidth_custom_zw", postfix);
		nvram_bufset(RT2860_NVRAM, "QoSUploadBandwidth_zw", upload_bandwidth);
		nvram_bufset(RT2860_NVRAM, "QoSDownloadBandwidth_zw", download_bandwidth);
...
*/	
leave:
	free_all(5, qos_enable, upload_bandwidth, download_bandwidth, 
			upload_bandwidth_custom, download_bandwidth_custom);
	return;
}

static void qos_rule(char *input)
{
	char rule[8192];
	const char *old_rule;
	char tmp[8];

	char *comment, *status, *mac_address, *max_upload_speed,*max_download_speed;
//	char *mac_address,
	char ingress_if[8];
	comment  = mac_address = max_upload_speed = max_download_speed = NULL;
//	mac_address = NULL;
	//web_debug_header();
	comment = strdup(web_get("comment", input, 0));	
	status = strdup(web_get("status", input, 0));
	mac_address = strdup(web_get("mac_address", input, 0));
//	ip_address = strdup(web_get("ip_address", input, 0));
	max_upload_speed  = strdup(web_get("QoSClassMaxUPSpeedStr_value", input, 0));
	max_download_speed = strdup(web_get("QoSClassMaxDLSpeedStr_value", input, 0));
	

	if(!comment || !mac_address || !max_upload_speed || !max_download_speed || !status) 
		goto leave;
	if(!strlen(status) || !strlen(mac_address)  || !strlen(max_upload_speed)  \
			|| !strlen(max_download_speed)|| strlen(comment) > 32)
		goto leave;
	...
	if(!old_rule || !strlen(old_rule))
		snprintf(rule, sizeof(rule), "%s,%s,%s,%s,%s", comment, status, mac_address,max_upload_speed,max_download_speed);
	else
		snprintf(rule, sizeof(rule), "%s;%s,%s,%s,%s,%s", old_rule, comment, status, mac_address,max_upload_speed,max_download_speed);

	nvram_bufset(RT2860_NVRAM, "QoSRules_zw", rule);
	nvram_commit(RT2860_NVRAM);
	qos_restart_zw();
  	web_back_parentpage();
  
leave:
	free_all(17, comment,status, mac_address, max_upload_speed,max_download_speed);

	return;
}

static void qos_delete_rules_zw(char *input)
{
	int i, j, rule_count;
	char name_buf[16];
	char *value;
...
*/
	...
	nvram_set(RT2860_NVRAM, "QoSRules_zw", new_rules);
	nvram_commit(RT2860_NVRAM);
...
int main(int argc, char **argv) 
{
	...
	//add by luozf
	else if (!strcmp(qos, "qos_zowee")){
		DBG_MSG();
		qos_zowee(inStr);
	}else if (!strcmp(qos, "qos_rule")){
		DBG_MSG();
		qos_rule(inStr);
	} else if (!strcmp(qos, "QoS_del_Rules")){
		qos_delete_rules_zw(inStr);
	}
	//end of luozf
	free(inStr);
