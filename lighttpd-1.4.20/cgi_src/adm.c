...
//add by luozf 2017-11-17
void set_ntp_hemu(char *input)
{
	
	//DBG_MSG("input=%s",input);
	char *time_radio=NULL;
	char *tz, *ntpServer, *ntpSync;

	
	time_radio = strdup(web_get("time_radio", input, 0));//0???????1??????
	
	//DBG_MSG("time_radio=%s",time_radio);
	if(atoi(time_radio)  == 0)
	{

	//'page=ymd=2017-11-17&hour=2&minute=3&second=2
	
		char *ymd, *hour, *mintue,*second;
		ymd = hour = mintue = second = NULL;
		char host_time[64] = {0};
		nvram_bufset(RT2860_NVRAM, "NTPMode", "0");
		ymd = strdup(web_get("ymd", input, 0));
		hour = strdup(web_get("hour", input, 0));
		mintue = strdup(web_get("minute", input, 0));	
		second = strdup(web_get("second", input, 0));

	
		if(strlen(ymd) > 0 && strlen(hour) > 0 && strlen(hour) > 0 && strlen(second) > 0)
		{	
			sprintf(host_time,"%s %s:%s:%s",ymd,hour,mintue,second);
		//	DBG_MSG("host_time=%s",host_time);
			ntp_sync_host(host_time);
			
			free_all(4,time_radio,ymd,hour,mintue,second);
			return;
		}
	}
	else
	{
		nvram_bufset(RT2860_NVRAM, "NTPMode", "1");
		ntpServer = ntpSync = NULL;
		...
		do_system("ntp.sh");
leave:
			free_all(4,tz, ntpServer, ntpSync,time_radio);
...
void load_default(void)
{
	...
	system("nvram_set factorydefault 1");
...
void set_opmode(char *input)
{
	...
	web_debug_header();
//	value = web_get("apcliSelect", input, 1);
//	nvram_set(RT2860_NVRAM, "apcliSelect",value);
/*	if(strcmp(value,"2860") == 0)
		{
		apcli2o5 = 2;
	}
	else if(strcmp(value,"rtdev") == 0)
		{
		apcli2o5 = 5;
	}*/
		...
		if (!strncmp(mode, "3", 2))
			{
				...
				system("iwpriv apcli0 set ApCliSsid=");
#if defined CONFIG_RTDEV
				nvram_bufset(RTDEV_NVRAM, "ApCliEnable", "1");
				system("iwpriv apclii0 set ApCliSsid=");
#endif			
			}
		else
			{
			nvram_bufset(RT2860_NVRAM, "apcliConnStatus", "Disconnect");
	...
	if (!strncmp(mode, "1", 2)) {
		//router mode, close dhcpfilter
		system("rmmod dhcpfilter.ko");
	} else { 
		// no router mode, open dhcpfilter	
...
int main(int argc, char *argv[]) 
{
	...
	//add by luozf 2017-11=17	
		...
		set_last_cmd(inStr);
	} else if (!strcmp(form, "OPMode") || !strcmp(form,"apclient")) {
		do_system("killall -9 ont_cloud;killall -9 andlink");
		set_opmode(inStr);
	}  else if (!strcmp(form,"routerMode")) {
		nvram_set(RT2860_NVRAM, "apcliConnStatus","Disconnect");
		set_opmode(inStr);
	} 
	else if (!strcmp(form, "loadreboot")) {
		load_reboot();
	}
	else if (!strcmp(form, "andlinkConfig")) {
		if(atoi(web_get("andlinkEnable", inStr, 0)) == 0)
			{
				system("killall -9 andlink");
				nvram_set(RT2860_NVRAM, "andlink_switch", "0");
			}
		else
			{
			nvram_set(RT2860_NVRAM, "andlink_switch", "1");
			}
	}
