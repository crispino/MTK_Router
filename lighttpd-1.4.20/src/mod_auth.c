...
static handler_t mod_auth_uri_handler(server *srv, connection *con, void *p_d) {
	...
	char *http_authorization = NULL, *http_referer =NULL;
	char *http_agent = NULL;
	data_string *ds,*ds_ua, *ds_rf;
	mod_auth_plugin_data *p = p_d;
	array *req;
    ...
    log_error_write(srv, __FILE__, __LINE__, "ss", "con->request.uri = ", (con->uri).path->ptr);

    //printf("con->request.uri = %s\n", con->request.uri);

	if(strstr((con->uri).path->ptr,"login.html") || strstr((con->uri).path->ptr,"llbg.png") || \
		strstr((con->uri).path->ptr,"loading.gif") || strstr((con->uri).path->ptr,"bodybg.jpg") || \
		strstr((con->uri).path->ptr,"mainstyle.css") || strstr((con->uri).path->ptr,"jquery.js") || \
		strstr((con->uri).path->ptr,"jquery.vegas.min.js") || strstr((con->uri).path->ptr,"base64.js") || \
		strstr((con->uri).path->ptr,"herouter.js") || strstr((con->uri).path->ptr,"and.css") || \
		strstr((con->uri).path->ptr,"jquery.vegas.css") || strstr((con->uri).path->ptr,"tinybox.js") || \
		//strstr((con->uri).path->ptr,"router") || 
		strstr((con->uri).path->ptr,"dy.png") || strcmp((con->uri).path->ptr,"/") == 0)
		
		return HANDLER_GO_ON;

	//过滤非重要内容
	if (!strstr((con->uri).path->ptr,".jsp") && !strstr((con->uri).path->ptr,".shtml")
		  && !strstr((con->uri).path->ptr, "main.html")) return HANDLER_GO_ON;

	//重要内容，需要过滤其Referer	
	if (NULL != (ds = (data_string *)array_get_element(con->request.headers, "Referer"))) {
		http_referer = ds->value->ptr;
		if (!strstr(http_referer, "login.html")) return HANDLER_GO_ON;				
	}
...
#define ZOWEE_REMOVE_401 1

#if ZOWEE_REMOVE_401
	if ((con->auth_success == 1) && (NULL == (ds = (data_string *)array_get_element(con->request.headers, "Authorization"))))
		return HANDLER_GO_ON;

	if (NULL != (ds_ua = (data_string *)array_get_element(con->request.headers, "User-Agent")) \
		&& (NULL == (ds = (data_string *)array_get_element(con->request.headers, "Authorization"))) \
		&& (NULL != (ds_rf = (data_string *)array_get_element(con->request.headers, "Referer")))) {
		http_agent = ds_ua->value->ptr;
		http_referer = ds_rf->value->ptr;
		if (strstr(http_agent, "Firefox") && strstr(http_referer, "login.html")) return HANDLER_GO_ON;
		
		log_error_write(srv, __FILE__, __LINE__, "ss", "http_agent", http_agent);
		log_error_write(srv, __FILE__, __LINE__, "sd", "auth_success ", con->auth_success);
	}
		...
		if (NULL != (auth_realm = strchr(http_authorization, ' '))) {
			...
			if ((auth_type_len == 5) &&
				...
				if (0 == strcmp(method->value->ptr, "basic")) {
					...
					con->auth_success = auth_satisfied;
				}
	...
	if (!auth_satisfied) {
		...
		con->file_finished = 1;
		con->mode = DIRECT;
		log_error_write(srv, __FILE__, __LINE__, "ss", "con->uri.path->ptr", (con->uri).path->ptr);		
		buffer_copy_string_len(p->tmp_buf, CONST_STR_LEN("/login.html"));
