...
int device_online_notify(const char *deviceId, const char *gwip)
{
	CREATE_URL(gwip, DEV_NOTICE);
	...
	cJSON_AddItemToObject(json, "deviceId", cJSON_CreateString(deviceId));
	cJSON_AddItemToObject(json, "eventType", cJSON_CreateString(EVENT_DEVICE_ONLINE));
	cJSON_AddItemToObject(json, "timestamp", cJSON_CreateNumber(router_getCompileTimeStamp()));

	cJSON_AddItemToObject(json, "data", data = cJSON_CreateObject());
	cJSON_AddItemToObject(data, "respCode", cJSON_CreateNumber(1));
	cJSON_AddItemToObject(data, "restCont", cJSON_CreateString("Success"));
	cJSON_AddItemToObject(data, "deviceType", cJSON_CreateString(g_dev_type));
	
	FILE *fp = fopen("/etc_ro/version", "r");
	char version[64] = {S_VERSION};
	if (fp != NULL) {fgets(version, sizeof(version), fp); version[strlen(version)-1] = '\0'; fclose(fp);}
	cJSON_AddItemToObject(data, "firmwareVersion", cJSON_CreateString(H_VERSION));
	cJSON_AddItemToObject(data, "softwareVersion", cJSON_CreateString(version)); //多个版本号则用分号隔开?

	char *ifname = NULL;
	ifname = get_ifname();
	cJSON_AddItemToObject(data, "ipAddress", cJSON_CreateString(get_ipaddr(ifname)));
	cJSON_AddItemToObject(data, "timestamp", cJSON_CreateNumber(router_getCompileTimeStamp()));
	cJSON_AddItemToObject(data, "deviceVendor", cJSON_CreateString(nvram_get(RT2860_NVRAM, "andlink_deviceVendor")));
#if defined (CONFIG_PRODUCT_341AA)
	cJSON_AddItemToObject(data, "deviceModel", cJSON_CreateString("HN120"));
#elif defined (CONFIG_PRODUCT_340AA)
	cJSON_AddItemToObject(data, "deviceModel", cJSON_CreateString("HN100"));
#elif defined (CONFIG_PRODUCT_339AA)
	cJSON_AddItemToObject(data, "deviceModel", cJSON_CreateString("HN350"));
#elif defined (CONFIG_PRODUCT_331AA)
	cJSON_AddItemToObject(data, "deviceModel", cJSON_CreateString("HN330"));
#else
	cJSON_AddItemToObject(data, "deviceModel", cJSON_CreateString("HN100"));
#endif

	cJSON_AddItemToObject(data, "deviceMac", cJSON_CreateString(g_dev_mac));

	char deviceSn[18] = "13111805000001"; //default
	router_getDevSn(deviceSn,  sizeof(deviceSn));
	cJSON_AddItemToObject(data, "deviceSn", cJSON_CreateString(deviceSn));

#if defined (CONFIG_PRODUCT_XXXAA)
	cJSON_AddItemToObject(data, "apUplinkType", cJSON_CreateString("PLC"));
#else
	if (3 == atoi(nvram_get(RT2860_NVRAM, "OperationMode")))
		cJSON_AddItemToObject(data, "apUplinkType", cJSON_CreateString("Wireless"));
	else
		cJSON_AddItemToObject(data, "apUplinkType", cJSON_CreateString("Ethernet"));
#endif

#if defined (CONFIG_PRODUCT_341AA)
	cJSON_AddItemToObject(data, "radio5", cJSON_CreateNumber(1));
#else
	cJSON_AddItemToObject(data, "radio5", cJSON_CreateNumber(0));
#endif
