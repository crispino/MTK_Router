...
char network_interface_name[8]="br0";
...
int get_results(void){
	...
	char *lanip = nvram_bufget(RT2860_NVRAM, "lan_ipaddr");
	char *dhcpserverok = nvram_bufget(RT2860_NVRAM, "dhcpserverok");//0: ok, others:nok
	char cmdset[64]={0},result_s[4]={0};
	int flag=atoi(dhcpserverok);
	
//printf("dhcpserverok:%d,result:%d\n",flag,result);
	if(result==STATE_OK)
		sprintf(result_s,"%s","0");
	else
		sprintf(result_s,"%s","1");

	nvram_set(RT2860_NVRAM, "dhcpserverok",result_s);

//printf("%d:%d\n",flag,atoi(result_s));
	if(flag == atoi(result_s) )
		{
			printf("dhcpserverok:no change\n");
			return result;
		}

	printf("dhcpserverok: change\n");
	...
	if (operationMode[0] == '1') {
	...
	else
		{
			if(result==STATE_OK)
			{
//				printf("Gateway exist, stop udhcpd\n");
				if(isforkexist("udhcpd") > 0)
					{
					system("killall udhcpd&");
					
					if(isforkexist("telnetd") == 0)  //if telnetd is running, do not restart network
						system("init_system restart");
					}
			}
			else
			{	
//				printf("NO gateway, startup udhcpd\n");
				if(isforkexist("udhcpd") <= 0)
					{
					if(isforkexist("telnetd") == 0)  //if telnetd is running, do not restart network
						system("init_system restart");
					...
					if(isforkexist("udhcpd") == 0)
						system("sleep 12 | udhcpd /etc/udhcpd.conf&");
					}
				else if(isforkexist("udhcpd") > 1)
					{	
					if(isforkexist("telnetd") == 0)  //if telnetd is running, do not restart network
						system("init_system restart");
