...
if [ "$opmode" = "3" -a "$CONFIG_MT7628_APCLI_SUPPORT$CONFIG_RT2860V2_AP_APCLI$CONFIG_RT3090_AP_APCLI$CONFIG_RT5392_AP_APCLI$CONFIG_RT5592_AP_APCLI$CONFIG_RT3593_AP_APCLI$CONFIG_MT7610_AP_APCLI$CONFIG_RT3572_AP_APCLI$CONFIG_RT5572_AP_APCLI$CONFIG_RT3680_iNIC_AP_APCLI$CONFIG_RTPCI_AP_APCLI$CONFIG_APCLI_SUPPORT" == "" ]; then
...
if [ "$opmode" = "2" -a "$CONFIG_RT2860V2_STA" == "" ]; then
	...
	factorydefault=`nvram_get 2860 factorydefault`

	telnetd_flag=`nvram_get 2860 telnetd_flag`
	
	if [ "$telnetd_flag" == "0" ];then
			//do nothing
	else
			telnetd &
	fi
...
if [ "$factorydefault" == "1" ];then
	...
	default_webpwd=`eth_mac g webpwd | cut -c 8-12`
	if [ ! -n "$default_webpwd" ]; then
		echo "=======default_webpwd is null========="
	else
		echo "====================default_webpwd=$default_webpwd"
		nvram_set 2860 Password $default_webpwd
	fi

	CONFIG_SSID_PREFIX=`nvram_get 2860 custom_ssid_prefix`

	## HN340 Or HN341
	if [ "$CONFIG_PRODUCT_340AA" == "y" -o "$CONFIG_PRODUCT_341AA" == "y" -o "$CONFIG_PRODUCT_340AA_JS" == "y" -o "$CONFIG_PRODUCT_341AA_JS" == "y" ] ;then
		default_mac=`ifconfig eth2 | grep HWaddr | cut -c 48-55 | sed 's/://g'`
		#echo "======CONFIG_SSID_PREFIX=$CONFIG_SSID_PREFIX,default_mac=$default_mac"
		default_ssid=${CONFIG_SSID_PREFIX}${default_mac}
		default_ssid_guest=${default_ssid}"-Guest"
		echo "====================default_ssid=$default_ssid"
		nvram_set 2860 SSID1 $default_ssid
		nvram_set 2860 SSID2 $default_ssid_guest
	
		default_psk_1=`eth_mac p 1 | cut -c 5-12`
		default_psk_2=`eth_mac p 2 | cut -c 5-12`

		nvram_set 2860 WPAPSK1 $default_psk_1
		nvram_set 2860 WPAPSK2 $default_psk_2
	
		##如果存在5G
		if [ "$CONFIG_RTDEV" == "y" ] ;then
			default_ssid=${CONFIG_SSID_PREFIX}2.4G-${default_mac}
			default_ssid_guest=${default_ssid}"-Guest"
			echo "====================default_ssid=$default_ssid"
			nvram_set 2860 SSID1 $default_ssid
			nvram_set 2860 SSID2 $default_ssid_guest
		
			default_ssid_5G=${CONFIG_SSID_PREFIX}5G-${default_mac}
			default_ssid_guest_5G=${default_ssid_5G}"-Guest"
			echo "====================default_ssid=$default_ssid_5G"
		
			nvram_set rtdev SSID1 $default_ssid_5G
			nvram_set rtdev SSID2 $default_ssid_guest_5G
		
			default_psk_1=`eth_mac p 1 | cut -c 5-12`
			default_psk_2=`eth_mac p 2 | cut -c 5-12`

			nvram_set rtdev WPAPSK1 $default_psk_1
			nvram_set rtdev WPAPSK2 $default_psk_2
		fi
	fi
	## HN350 Or HN330
	if [ "$CONFIG_PRODUCT_339AA" == "y" -o "$CONFIG_PRODUCT_331AA" == "y" -o "$CONFIG_PRODUCT_339AA_JS" == "y" -o "$CONFIG_PRODUCT_331AA_JS" == "y" ] ;then
		default_mac=`ifconfig eth2 | grep HWaddr | cut -c 51-55 | sed 's/://g'`
		#echo "======CONFIG_SSID_PREFIX=$CONFIG_SSID_PREFIX,default_mac=$default_mac"
		default_ssid=${CONFIG_SSID_PREFIX}${default_mac}
		#default_ssid_guest=${default_ssid}"-Guest"
		echo "====================default_ssid=$default_ssid"
	...
	nvram_set 2860 factorydefault "0"
fi
...
#   3 = AP Client
#
if [ "$opmode" = "0"  -a "$VLANEnable" = "0" ]; then
	configVIF
...
# RTDEV_MII support: start mii iNIC after network interface is working
...
elif [ "$opmode" = "1" ] || [ "$opmode" = "0" -a "$VLANEnable" = "1" ]; then
	configVIF
	if [ "$CONFIG_RAETH_ROUTER" = "y" -a "$CONFIG_LAN_WAN_SUPPORT" = "y" ]; then
		if [ "$CONFIG_WAN_AT_P0" = "y" ]; then
	...
	##配置WAN vlan
	if [ "$opmode" = "1" -a "$VLANEnable" = "1" ] ; then
		echo "++++++++++Seven_debug+++++++++route+++++++++"
		config_vlan_custom.sh route
	elif [ "$opmode" = "0" -a "$VLANEnable" = "1" ] ; then
		echo "++++++++++Seven_debug+++++++++bridge+++++++++"
		config_vlan_custom.sh bridge
	fi
...
ApCliEnable2=`nvram_get 2860 ApCliEnable`
ApCliEnable5=`nvram_get rtdev ApCliEnable`

if [ "$ApCliEnable2" = "1" ]; then
	iwpriv apcli0 set ApCliAutoConnect=1
	iwpriv apcli0 set ApCliEnable=0
	iwpriv apcli0 set ApCliEnable=1
fi

if [ "$ApCliEnable5" = "1" ];then
	iwpriv apclii0 set ApCliAutoConnect=1
	iwpriv apclii0 set ApCliEnable=0
	iwpriv apclii0 set ApCliEnable=1
fi

wifi_off2=`nvram_get 2860 WiFiOff`
if [ "$wifi_off2" = "1" ];then
	ifconfig ra0 down
	ifconfig ra1 down
	ifconfig ra2 down
...
wifi_off5=`nvram_get rtdev WiFiOff`
if [ "$wifi_off5" = "1" ];then
	ifconfig rai0 down
	ifconfig rai1 down
	ifconfig rai2 down
